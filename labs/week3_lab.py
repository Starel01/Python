...
# -- ANCHOR QUESTIONS ------------------------------------------------------
# Q1. Why is a tuple ('A', '.-') safer here than a list ['A', '.-']?A tuple is safer because they can't be changed unlike lists.
#     (Hint: what should NEVER change about a codebook entry?)
# Q2. "for letter, pattern in CODEBOOK" unpacks each tuple into two names.
#     How is that different from "for entry in CODEBOOK"?"" For entry in codebook" the whole tuple is in entry.
# Q3. CODEBOOK[:8] is a slice. What would CODEBOOK[-3:] give you? It would give us the last three entries.
...
encoded = []
for character in word:
    positions = letters.index(character)
    encoded.append(patterns[positions])
print(encoded)
...
for letter, pattern in CODEBOOK:
    ...
    if letter == 'z':
        break
print(f"counter needed to reach 'z': {counter}")
